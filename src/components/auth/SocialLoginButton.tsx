...
export default function SocialLoginButton({ provider, className = "", ...props }: SocialLoginButtonProps) {
  ...
  return (
    <motion.button
      ...
      className={`w-full py-3 px-4 text-xs font-semibold uppercase tracking-wider rounded-xl cursor-pointer select-none transition-all duration-300 flex items-center justify-center gap-2.5 outline-none glass-panel border border-theme-glass-border hover:border-theme-text-primary/40 hover:bg-theme-text-primary/5 text-theme-text-primary ${className}`}
      {...props}
    >
      {isGoogle && (
        <>
      ...
      {isMicrosoft && (
        <>
          ...
          <span className="font-sans">Microsoft</span>
        </>
      )}
