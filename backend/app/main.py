...
from app.database import init_db, get_db, User, Meeting, Participant, Message, File, Recording, Attendance, OTPCode, RefreshToken, UserSettings, Notification
...
from app.email_service import send_welcome_email, send_forgot_password_email, send_password_changed_email
...
async def upload_recording(
    ...
    try:
        ...
        if meeting and meeting.host_id:
            ...
            if host:
                from app.email_service import send_recording_ready_email
...
async def translate_text(
    ...
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key={clean_gemini_key}"
            ...
            if res.status_code == 200:
                ...
                try:
                    ...
                    if meeting and meeting.host_id:
                        host = db.query(User).filter(User.id == meeting.host_id).first()
                        if host:
                            from app.email_service import send_ai_summary_email
...
def invite_to_meeting(
    ...
    from app.email_service import send_meeting_invitation_email
...
def remind_meeting(
    ...
    from app.email_service import send_meeting_reminder_email
